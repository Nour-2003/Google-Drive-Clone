  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<HomeCubit, HomeStates>(
      ...
      builder: (context, state) {
        ...
        if (state is HomeFilterState) {
          showFiles = state.showFiles;
          showFolders = state.showFolders;
        }

        final filteredItems = displayItems.where((item) {
          if (item['isFolder'] == true) return showFolders;
          return showFiles;
        }).toList();

        final folders =
            filteredItems.where((item) => item['isFolder'] == true).toList();
        final files =
            filteredItems.where((item) => item['isFolder'] == false).toList();
        return Scaffold(
